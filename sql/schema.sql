...
CREATE INDEX idx_packages_status_destination ON packages(status, destination);
CREATE INDEX idx_packages_slug ON packages(slug);
CREATE INDEX idx_package_itineraries_package_day ON package_itineraries(package_id, day_number);
CREATE INDEX idx_package_bookings_user_created ON package_bookings(user_id, created_at DESC);
CREATE INDEX idx_package_bookings_package_created ON package_bookings(package_id, created_at DESC);

-- AI itinerary generation extensions
CREATE TABLE ai_generated_packages (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_ai_generated_packages_status_created ON ai_generated_packages(status, created_at DESC);
CREATE INDEX idx_ai_generated_packages_created_by ON ai_generated_packages(created_by, created_at DESC);
CREATE INDEX idx_ai_generation_logs_created ON ai_generation_logs(created_at DESC);

-- Destinations & Attractions Platform
