        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {DESTINATION_CATEGORIES.map((category) => (
            <Link
              key={category}
              href={
                `/attractions?category=${encodeURIComponent(category.toLowerCase())}` as Route
              }
              className="rounded-full border px-3 py-1 text-sm font-medium"
            >
              {category}
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        ...
        <div className="space-y-4">
          <AIRecommendationsPanel
            ...
            items={ai.top_5.map((item: any) => ({
              name: item.name,
              category: item.category || "culture",
              score: item.score || 95,
              reason: item.reason,
              bestTime: item.bestTime || item.best_time,
            }))}
          />
