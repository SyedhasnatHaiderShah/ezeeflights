...
const COMPARISON_CRITERIA = [
  ...
  { label: "Price / Night", key: "minPricePerNight", type: "price" },
  {
    ...
    key: "rooms.0.freeCancellation",
    type: "boolean",
  },
  {
    label: "Breakfast Included",
    key: "rooms.0.breakfastIncluded",
    type: "boolean",
  },
...
function HotelComparePageContent() {
        ...
        <div className="mx-auto max-w-screen-2xl px-4 md:px-6">
          ...
          <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-4">
              ...
              >
                <ArrowLeft className="w-3.5 h-3.5" /> {t("Back to Search")}
              </Link>
              <h1 className="text-3xl md:text-4xl font-bold tracking-tight">
                {t("Compare Properties")}
              </h1>
              <p className="text-sm text-muted-foreground font-medium">
                {t("Analyze side-by-side to find your perfect stay.")}
              </p>
            </div>
          ...
          {hotelsToCompare.length === 0 ? (
            <div className="py-20 text-center space-y-6 bg-card/30 backdrop-blur-md rounded-[3rem] border border-border/50">
              ...
              <div className="space-y-2">
                <p className="text-xl font-bold">{t("No hotels selected")}</p>
                <p className="text-sm text-muted-foreground">
                  {t("Select up to 3 hotels from the search results to compare them.")}
                </p>
              </div>
              <Link
                href="/hotels/search"
                className="inline-block bg-brand-red text-white px-8 py-3.5 rounded-2xl font-bold text-xs shadow-xl shadow-brand-red/20"
              >
                {t("Go to Search")}
              </Link>
            </div>
              ...
              <div className="min-w-[800px]">
                ...
                <div className="grid grid-cols-[200px_repeat(3,1fr)] gap-6 mb-10">
                  ...
                  {hotelsToCompare.map((hotel) => (
                    <div
                      key={hotel.id}
                      className="group relative space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700"
                    >
                      <div className="aspect-[4/3] rounded-[2.5rem] overflow-hidden border border-border/50 shadow-xl group-hover:border-brand-red/30 transition-all duration-500">
                        ...
                        {hotel.isBestForTrip && (
                          <div className="absolute top-4 left-4 bg-brand-red text-white text-[9px] font-bold px-3 py-1.5 rounded-full shadow-2xl flex items-center gap-1.5">
                            <Sparkles className="w-3 h-3 fill-current" /> {t("AI Top Pick")}
                          </div>
                        )}
                      </div>
                      <div className="px-2 space-y-1">
                        <h3 className="font-bold text-lg leading-tight group-hover:text-brand-red transition-colors">
                          {hotel.name}
                        </h3>
                  ...
                  {[...Array(3 - hotelsToCompare.length)].map((_, i) => (
                    <div
                      key={i}
                      className="rounded-[2.5rem] border-2 border-dashed border-border/50 bg-muted/10 flex flex-col items-center justify-center p-8 text-center space-y-4 opacity-50"
                    >
...
// Suspense-wrapped
export default function HotelComparePage(props: any) {
  return (
    <Suspense fallback={null}>
      <HotelComparePageContent {...props} />
    </Suspense>
  );
}
