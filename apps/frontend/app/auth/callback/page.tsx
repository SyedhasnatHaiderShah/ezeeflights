...
function OAuthCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (!code) {
      setMessage('Missing sign-in code. Open the link from the login page again.');
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setMessage('This sign-in link expired or was already used. Try Google again from login.');
        }
      }
    })();
...
export default function OAuthCallbackPage() {
  return (
    <section className="mx-auto max-w-md rounded-xl bg-white p-8 shadow">
      <h1 className="mb-4 text-xl font-semibold text-slate-900">Finishing sign-in</h1>
