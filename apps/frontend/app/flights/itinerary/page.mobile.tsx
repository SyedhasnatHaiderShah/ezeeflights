...
function BookingPageContent() {
  ...
  const [isSelectVerified, setIsSelectVerified] = useState(() => {
    ...
    const isBid = params.get("bid") === "true" || flightId?.includes("cheap-bid");
  ...
  useEffect(() => {
    ...
    if (flightId.includes(",")) {
      ...
      Promise.all(ids.map((id) => apiFetch(`/flights/${id}`)))
        ...
        .catch((err) => {
          console.error("[GetFlightDetails] Error fetching multi-city flight details:", err);
          toast({
      ...
      apiFetch(`/flights/${flightId}`)
        ...
        .catch((err) => {
          console.error("[GetFlightDetails] Error fetching flight details:", err);
          toast({
            title: t("Failed to load flight details"),
            description: t("Please check your connection or refresh the page."),
            variant: "destructive",
    ...
    return {
      ...
      flightFare: isBid
        ? (() => {
            ...
            return {
              adultFare: appliedMeta && appliedMeta.bidAdtPrice !== undefined
                ? appliedMeta.bidAdtPrice
                : (standbyDeal?.bidAdtPrice ?? 0),
              childFare: appliedMeta && appliedMeta.bidChdPrice !== undefined
                ? appliedMeta.bidChdPrice
                : (standbyDeal?.bidChdPrice ?? standbyDeal?.bidAdtPrice ?? 0),
              infantFare: appliedMeta && appliedMeta.bidInfPrice !== undefined
                ? appliedMeta.bidInfPrice
                : (standbyDeal?.bidInfPrice ?? 0),
              adultTax: 0,
              childTax: 0,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (e.message === "Payment cancelled by user") {
        ...
        if (false) {
          userFriendlyMsg = userFriendlyMsg.replace(
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <main className="flex-grow container mx-auto max-w-7xl px-4 pt-20 pb-24 md:pb-8">
        ...
        >
          ...
          {step < 2 && (
            <aside className="hidden lg:block space-y-6 self-start order-1 lg:order-2 w-full lg:w-[380px]">
              <div className="lg:sticky lg:top-24 lg:z-10">
                {subStep === 3 && tripSummaryElement}
              </div>
            </aside>
          )}
        </div>
