...
import { FlightStickySearchPanel } from "./FlightStickySearchPanel";
import { DestinationInsightsPrefetch } from "./DestinationInsightsPrefetch";
...
async function fetchFlights(
  ...
  let rawClass = apiParams.get("prefClass") || apiParams.get("class") || apiParams.get("cabin");
  if (rawClass) {
  ...
  apiParams.delete("cabin");

  // Ensure trip and loading are present for backend request routing
  if (!apiParams.has("trip")) {
    const hasReturn = apiParams.has("returnDate") || apiParams.has("rDate") || apiParams.has("retDate");
    apiParams.set("trip", hasReturn ? "round-trip" : "one-way");
  }
  if (!apiParams.has("loading")) {
    apiParams.set("loading", "false");
  }
  ...
  try {
    ...
    return {
      ...
      fromCache: resData.fromCache === true,
      cachedAt: resData.cachedAt,
      error: resData.error,
      suggestedCabin: resData.suggestedCabin,
    };
  } catch (error: any) {
...
async function FlightResultsWrapper({
  ...
  const { flights, total, airlines, fromCache, cachedAt, error: apiError, suggestedCabin } = await fetchFlights(fetchParams, frontendOrigin || undefined);
  const currentPage = parseInt(searchParams.get("page") || "1");
  const loadingParam = searchParams.get("loading");
  ...
  return (
    <div className="h-screen bg-background text-foreground flex flex-col overflow-hidden max-md:h-auto max-md:overflow-y-auto">
      ...
      <div className="w-full max-w-[1440px] mx-auto px-4 md:px-5 mt-12 pt-6 pb-0 flex-1 min-h-0 overflow-hidden max-md:overflow-visible max-md:h-auto">
        <GlobalLoaderTrigger message="Searching flights..." heroMode={true} />
