...
export function AiSuggestionsPanel({ flights, origin, destination, onClose }: Props) {
  ...
  return (
    <aside className="w-full flex flex-col gap-4 p-4 h-full overflow-y-auto bg-slate-50/50 dark:bg-background/50">
      ...
      {bestPick && (
        ...
        >
          ...
          <h3 className="font-bold text-sm mb-1">{bestPick.airline.name} to {destination}</h3>
          <p className="text-xs text-muted-foreground mb-4">
            Optimized for both price and duration. A rare balance of efficiency.
          </p>
          <div className="flex items-center justify-between border-t border-border pt-3">
            <div className="text-lg font-black text-redmix">${bestPick.totalCost}</div>
            <div className="text-xs font-semibold text-muted-foreground">{Math.floor(bestPick.totalTime/60)}h {bestPick.totalTime%60}m</div>
          </div>
        </motion.div>
      )}
      ...
      <div className="rounded-2xl bg-white dark:bg-card border border-border p-4 shadow-sm">
        ...
        <div className="flex items-center gap-4 mb-4">
          <div className="flex-1 h-2 bg-slate-100 dark:bg-muted rounded-full overflow-hidden flex">
            <div className={cn("h-full", priceStatus === 'low' ? "w-1/3 bg-emerald-500" : "w-1/3 bg-slate-200 dark:bg-muted-foreground/20")} />
            <div className={cn("h-full border-x border-white dark:border-card", priceStatus === 'typical' ? "w-1/3 bg-yellow-500" : "w-1/3 bg-slate-200 dark:bg-muted-foreground/20")} />
            <div className={cn("h-full", priceStatus === 'high' ? "w-1/3 bg-redmix" : "w-1/3 bg-slate-200 dark:bg-muted-foreground/20")} />
          </div>
        </div>
          ...
          {priceStatus === 'low' ? (
            <>
              ...
              <p className="text-xs text-emerald-700 dark:text-emerald-400 font-medium">
                Prices are currently <span className="font-bold">Low</span> for this route. It's a great time to book!
              </p>
            </>
          ) : priceStatus === 'high' ? (
            <>
              ...
              <p className="text-xs text-red-700 dark:text-red-400 font-medium">
                Prices are <span className="font-bold">Higher</span> than usual. Consider flexible dates.
              </p>
            </>
          ) : (
            <>
              ...
              <p className="text-xs text-blue-700 dark:text-blue-400 font-medium">
                Prices are <span className="font-bold">Typical</span> for this route. Consistent value found.
              </p>
            </>
          )}
        </div>
      ...
      <div className="rounded-2xl bg-white dark:bg-card border border-border p-4 shadow-sm">
        ...
        <p className="text-xs text-muted-foreground mb-3 leading-relaxed">
          Flying on a <span className="text-foreground font-semibold">Wednesday</span> could save you up to <span className="text-redmix font-bold">$120</span> on this route.
        </p>
        <Button variant="outline" size="sm" className="w-full text-xs h-8 border-redmix/20 text-redmix hover:bg-redmix/5">
          View Price Calendar
        </Button>
      </div>
      ...
      <div className="mt-auto rounded-2xl bg-redmix p-4 shadow-lg text-white">
        ...
        <div className="relative">
          ...
          />
          <Button size="icon" className="absolute right-1 top-1 h-6 w-6 bg-white text-redmix hover:bg-white/90">
