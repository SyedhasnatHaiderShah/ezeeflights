...
function ConfirmationPageContent() {
    ...
    return (
      <section className="mx-auto max-w-xl p-8 text-center">
        ...
        <p className="mt-3 text-gray-600">
          {t("No booking reference provided.")}
        </p>
  ...
  if (isLoading) {
    return (
      <section className="mx-auto max-w-xl p-5 text-center">
  ...
  if (!booking) {
    return (
      <section className="mx-auto max-w-xl p-5 text-center space-y-3">
        <h1 className="text-2xl font-bold">{t("Unable to load booking")}</h1>
        <button
          onClick={() => refetch()}
          className="rounded bg-brand-red px-4 py-2 text-white"
        >
          {t("Retry")}
        </button>
      </section>
  ...
  return (
    <section className="mx-auto max-w-4xl px-4 py-10 space-y-8">
      ...
      <div className="rounded-2xl border bg-card p-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xl font-semibold">
              {t("Your itinerary is confirmed and ticketed.")}
            </p>
          </div>
        ...
        <div className="mt-4 grid sm:grid-cols-3 gap-3 text-sm">
          ...
          <div className="rounded-xl border p-3">
            ...
            <p className="font-medium capitalize">
              {t(booking.paymentStatus.toLowerCase())}
            </p>
          </div>
          <div className="rounded-xl border p-3">
            <p className="text-muted-foreground">{t("Booked on")}</p>
      ...
      <div className="space-y-3">
        <h2 className="text-xl font-semibold">{t("What's next")}</h2>
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            { title: "Download E-ticket", icon: Download },
            { title: "Add to Calendar", icon: CalendarPlus },
            { title: "Share Itinerary", icon: Share2 },
          ].map(({ title, icon: Icon }) => (
            <div
              key={title}
            ...
            >
              ...
              <p className="text-sm text-muted-foreground">
                {t("Manage your booking and keep all travel details handy.")}
              </p>
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          href="/my-trips"
          className="rounded-lg bg-brand-red text-white px-4 py-2"
        >
          {t("View My Trips")}
        </Link>
        <Link href="/hotels" className="rounded-lg border px-4 py-2">
          {t("Book a Hotel")}
        </Link>
      </div>
    </section>
