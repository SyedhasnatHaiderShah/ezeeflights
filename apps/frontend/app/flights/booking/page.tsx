...
import { Check, ChevronRight, User as UserIcon, Info, CreditCard } from 'lucide-react';
        ...
        <div className="mb-8 flex flex-col items-center text-center">
            ...
            {step < 2 && (
                <div className="w-full max-w-2xl">
                    <Progress value={Math.min(progressValue, 100)} className="h-1.5 bg-muted" />
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_380px]">
          <div className="space-y-8">
                ...
                <motion.div key={step} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                    ...
                    {step === 1 && (
                        <Card className="rounded-2xl border-border bg-card shadow-xl overflow-hidden">
                            <CardContent className="p-8 md:p-12 text-center">
                                ...
                                <h2 className="text-3xl font-black uppercase mb-3 tracking-tight">Ready to Confirm?</h2>
                                <p className="text-muted-foreground text-sm mb-8">Review your flight details and complete your secure payment to finalize the booking.</p>
                                <Button 
                                    onClick={nextStep} 
                                    disabled={loading}
                                    size="lg" 
                    ...
                    {step === 2 && (
                        ...
                        >
                            ...
                            <h2 className="text-3xl font-black text-foreground mb-4 tracking-tight uppercase">Booking Confirmed!</h2>
                            <p className="text-muted-foreground text-sm mb-8 max-w-lg mx-auto">
                                Thank you for choosing EzeeFlights. Your journey to <span className="text-foreground font-bold">{flightDetails?.destination}</span> is officially locked in.
                            </p>
            ...
            <div className="flex items-center justify-between mt-6">
                <Button variant="ghost" disabled={step === 0 || step === 2} onClick={() => setStep(s => s - 1)} className="text-muted-foreground hover:text-foreground text-xs font-bold uppercase">
                    ← Previous Step
                </Button>
                {step < 1 && (
          ...
          <aside className="space-y-6">
            <Card className="sticky top-24 rounded-2xl border-border bg-card shadow-lg overflow-hidden">
              <CardContent className="p-6">
                <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Trip Summary</h3>
