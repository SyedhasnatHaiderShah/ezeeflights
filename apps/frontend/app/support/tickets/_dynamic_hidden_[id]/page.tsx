'use client';
export const dynamic = "force-static"; // @capacitor-dynamic-override @capacitor-build-toggle
...
export default function TicketDetailPage() {
  ...
  const ticket = useQuery({
    ...
    queryFn: () =>
      apiFetch<SupportTicket & { messages: Array<any> }>(
  ...
  const closeMutation = useMutation({
    mutationFn: (payload: { rating?: number; comment?: string }) =>
      apiFetch(`/support/tickets/${params.id}/close`, {
        method: "POST",
        body: JSON.stringify(payload),
      }),
      ...
      <div className="rounded-lg border bg-white p-5">
        ...
        {remainingHours !== null && (
          <p className="mt-2 text-sm text-indigo-700">
            SLA timer: {remainingHours}h remaining
          </p>
        )}
      </div>
        ...
        <div className="space-y-3">
          ...
          <SatisfactionRating
            onSubmit={async (rating, comment) => {
              await closeMutation.mutateAsync({ rating, comment });
            }}
          />
        </div>
      )}
