...
export default function CheckoutPage() {
  ...
  const [promoCode, setPromoCode] = useState(searchParams.get('promo') ?? searchParams.get('code') ?? '');
  const debouncedPromoCode = useDebouncedValue(promoCode.trim().toUpperCase(), 450);
  ...
  const bookingQuery = useQuery<{ totalAmount: number; totalPrice?: number; currency: string }>({
  ...
  const total = bookingQuery.data?.totalPrice ?? bookingQuery.data?.totalAmount ?? 0;
  ...
  useEffect(() => {
    ...
    const update = () => setFlashCountdown(formatCountdown(new Date(endsAt).getTime() - Date.now()));
    update();
    const timer = window.setInterval(update, 1000);
    return () => window.clearInterval(timer);
  }, [promoQuery.data?.flashSaleEndsAt]);

  const activeCampaigns = campaignsQuery.data ?? promoQuery.data?.recommendations ?? [];
    ...
    try {
      const data = (await initiatePayment({
        ...
        metadata: {
          ...
          discountAmount,
          appliedPromotions: promoQuery.data?.appliedDiscounts?.map((item) => item.code) ?? [],
    ...
    <main className="mx-auto grid max-w-7xl gap-4 p-6 lg:grid-cols-[5fr_3fr_2fr]">
      <section className="space-y-4 rounded-2xl border p-5">
        <div>
          ...
          <p className="text-sm text-slate-500">Cards, wallets, BNPL, and discount-aware checkout.</p>
        </div>
        ...
        <div className="rounded-xl border p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <span className={`rounded-full border px-2 py-1 text-[11px] font-semibold ${validationTone}`}>
              {promoQuery.isFetching ? 'Validating…' : promoQuery.data?.promoStatus ?? 'Waiting'}
            </span>
          </div>
          <input
            value={promoCode}
          ...
          />
          <p className="mt-2 text-xs text-slate-500">{promoQuery.data?.promoMessage ?? 'Discounts update in real time as you type.'}</p>
          {flashCountdown ? (
          ...
          {activeCampaigns.length > 0 ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {activeCampaigns.map((campaign) => (
                <span key={campaign.code} className="rounded-full border bg-slate-50 px-3 py-1 text-xs font-medium text-slate-600">
                  {campaign.title}
                </span>
              ))}
            </div>
        ...
        <div className="rounded-xl border p-3">
          <p className="mb-2 text-sm font-medium">Credit / Debit Card (Stripe)</p>
          <StripeCheckout onPay={startPayment} loading={loading} />
        </div>

        <div className="rounded-xl border p-3">
          <p className="mb-2 text-sm font-medium">PayPal</p>
          <button className="rounded bg-[#0070ba] px-4 py-2 text-white">Pay with PayPal</button>
        </div>

        <div className="rounded-xl border p-3">
          <p className="mb-2 text-sm font-medium">Wallet Balance</p>
          <CheckoutSummary
            subtotal={total}
            currency={currency}
      ...
      <section className="space-y-4 rounded-2xl border p-5">
        ...
        <div className="rounded-xl border p-3 text-sm">
          <p>Flight + Hotel</p>
          <p className="text-slate-500">Booking #{bookingId || 'N/A'}</p>
        </div>

        <div className="space-y-2 text-sm">
          <p className="flex justify-between"><span>Subtotal</span><span>{currency} {total.toFixed(2)}</span></p>
          <p className="flex justify-between"><span>Promo discount</span><span>- {currency} {discountAmount.toFixed(2)}</span></p>
          <p className="flex justify-between"><span>Wallet</span><span>- {currency} {walletApplied.toFixed(2)}</span></p>
          <p className="flex justify-between font-bold"><span>Total</span><span>{currency} {payableAmount.toFixed(2)}</span></p>
        </div>

        <div className="rounded-xl border bg-slate-50 p-3 text-xs text-slate-600">
          <p className="font-semibold text-slate-800">Automatic discounts</p>
          <ul className="mt-2 space-y-1">
            {promoQuery.data?.appliedDiscounts?.length ? (
              promoQuery.data.appliedDiscounts.map((item) => (
                <li key={item.code}>
                  {item.title} — {currencySymbol(currency)}{item.amount.toFixed(2)}
                </li>
              ))
            ) : (
              <li>No automatic discounts applied yet.</li>
            )}
          </ul>
        </div>
      </section>
      ...
      <aside className="space-y-3 rounded-2xl border p-5 text-sm">
        ...
        <p>Support: support@ezeeflights.com</p>
      </aside>
    </main>
  );
}
