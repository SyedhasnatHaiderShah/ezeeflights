...
export function CheckoutSummary({ total, walletBalance, useWallet, onToggleWallet }: Props) {
  ...
  return (
    <div className="rounded-lg border bg-white p-4 shadow-sm">
      ...
      <label className="mb-3 flex items-center justify-between rounded border p-3">
      ...
      <p className="text-sm text-gray-600">Available balance: ${walletBalance.toFixed(2)}</p>
      <div className="mt-3 space-y-1 text-sm">
        ...
        <p className="flex justify-between font-semibold"><span>Card charge</span><span>${remaining.toFixed(2)}</span></p>
      </div>
      {walletApplied > 0 && remaining > 0 && <p className="mt-3 rounded bg-emerald-50 p-2 text-xs font-medium text-emerald-700">Split Payment</p>}
    </div>
  );
}
