...
const DestinationCard = React.memo(function DestinationCard({ dest }: { dest: Destination }) {
  return (
    <Link
  ...
  return (
    <section className="py-12 lg:py-20 bg-background relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="flex items-end justify-between mb-8 lg:mb-12 text-foreground">
          ...
          <Link href="/destinations" className="text-xs font-black text-brand-red hover:text-brand-red/80 uppercase tracking-[0.2em] transition-colors border-b-2 border-brand-red/20 pb-0.5">
            Explore All
          </Link>
        </div>
        ...
        <div className="md:hidden">
          <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 -mx-6 px-6 pb-6 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            {DESTINATIONS.map((dest) => (
              <div key={dest.id} className="w-[85vw] max-w-[320px] shrink-0 snap-start">
