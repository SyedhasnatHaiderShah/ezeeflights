    ...
    <React.Suspense fallback={null}>
      <div className="flex min-h-screen flex-col bg-background text-foreground">
        ...
        <main className="flex-grow pt-24">
          <section className="bg-gradient-to-r from-[#072f66] to-[#1d4f91] px-6 py-16 text-white">
            <div className="mx-auto max-w-6xl space-y-4">
              <p className="text-sm uppercase tracking-[0.2em] text-white/70">
                Airline Partner
              </p>
              <h1 className="text-5xl font-black">Fly with {data.intro.title}</h1>
              <p className="max-w-2xl text-white/85">{data.hero.description}</p>
            </div>
          </section>
            ...
            <Tabs defaultValue="overview" className="space-y-5">
              ...
              <TabsContent value="deals">
                <div className="grid gap-3 md:grid-cols-4">
                  {["Seats", "Meals", "Entertainment", "WiFi"].map((a) => (
                    <div key={a} className="rounded-xl border p-4 text-center">
                      ✈️ {a}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {data.sections.map((s) => (
                    <div key={s.title}>
                      ...
                      <p className="text-sm text-slate-600">{s.paragraphs[0]}</p>
                    </div>
                  ))}
                </div>
              </TabsContent>
