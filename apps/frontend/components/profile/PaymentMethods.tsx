  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-foreground">{t("Payment Methods")}</h3>
          <p className="text-sm text-muted-foreground">{t("Manage your saved cards and billing options")}</p>
        </div>
          ...
          <DialogContent className="sm:max-w-[425px] overflow-hidden">
            ...
            <form onSubmit={handleAddCard} className="space-y-5 py-2">
              <div className="space-y-2">
                <Label htmlFor="card-name" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("Cardholder Name")}</Label>
                <Input 
                  id="card-name" 
              ...
              <div className="space-y-2">
                <Label htmlFor="card-number" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("Card Number")}</Label>
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-2">
                  <Label htmlFor="expiry" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("Expiry Date")}</Label>
                  <Input 
                    id="expiry" 
                    placeholder="MM / YY" 
                    className="h-12 bg-muted/30 border-muted-foreground/20 focus:bg-background transition-all"
                ...
                <div className="space-y-2">
                  <Label htmlFor="cvc" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{t("CVC / CVV")}</Label>
                  <div className="relative">
                    <Input 
                      id="cvc" 
                      placeholder="•••" 
              ...
              <div className="flex items-center gap-3 rounded-xl bg-emerald-500/5 p-4 text-[11px] leading-relaxed text-emerald-700 border border-emerald-500/10 dark:bg-emerald-500/10 dark:text-emerald-400">
                ...
                <p>{t("Your payment data is processed securely via SSL encryption. We never store your full card details.")}</p>
              </div>
              <DialogFooter className="pt-2">
                <Button type="button" variant="ghost" className="rounded-xl" onClick={() => setIsAddOpen(false)}>{t("Cancel")}</Button>
                <Button type="submit" className="bg-redmix hover:bg-brand-red-light rounded-xl h-11 px-8 shadow-lg shadow-redmix/20">{t("Add Card")}</Button>
              </DialogFooter>
            </form>
          </DialogContent>
        </Dialog>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
          ...
          >
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 flex items-center justify-between">
                ...
                <p className="text-[10px] text-muted-foreground">{t("Expires")} {c.e}</p>
              </div>
            </div>
          </div>
        ))}
        ...
        {cards.length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center rounded-2xl border border-dashed border-border py-12 text-center">
            ...
            <p className="mt-4 font-medium text-foreground">{t("No cards saved yet")}</p>
            <p className="text-sm text-muted-foreground">{t("Add a card to speed up your booking process")}</p>
          </div>
        )}
      </div>
    </div>
