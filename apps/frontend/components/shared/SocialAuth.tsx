import * as React from "react"
import { Button } from "@/components/ui/button"
import { Mail } from "lucide-react"

export function GoogleIcon({ className }: { className?: string }) {
  return (
    <svg 
      viewBox="0 0 24 24" 
      className={className} 
      xmlns="http://www.w3.org/2000/svg"
    >
      <path 
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" 
        fill="#4285F4" 
      />
      <path 
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-1 .67-2.28 1.07-3.71 1.07-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" 
        fill="#34A853" 
      />
      <path 
        d="M5.84 14.11c-.22-.67-.35-1.39-.35-2.11s.13-1.44.35-2.11V7.05H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.95l3.66-2.84z" 
        fill="#FBBC05" 
      />
      <path 
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.05l3.66 2.84c.87-2.6 3.3-4.51 6.16-4.51z" 
        fill="#EA4335" 
      />
    </svg>
  )
}

export function SocialAuth({ onEmailClick }: { onEmailClick?: () => void }) {
  return (
    <div className="space-y-4 w-full">
      <Button 
        variant="outline" 
        className="w-full h-11 rounded-xl flex items-center justify-center gap-3 border-gray-200 dark:border-border text-foreground font-semibold hover:bg-muted/50 transition-all shadow-sm"
      >
        <GoogleIcon className="w-5 h-5" />
        <span className="text-sm">Google</span>
      </Button>

      <div className="relative py-1">
        <div className="absolute inset-0 flex items-center">
          <span className="w-full border-t border-border" />
        </div>
        <div className="relative flex justify-center text-[10px] uppercase">
          <span className="bg-background px-2 text-muted-foreground font-medium lowercase tracking-widest">
            or
          </span>
        </div>
      </div>

      <Button 
        variant="outline" 
        onClick={onEmailClick}
        className="w-full h-11 rounded-xl flex items-center justify-center gap-3 border-gray-200 dark:border-border text-foreground font-semibold hover:bg-muted/50 transition-all shadow-sm"
      >
        <Mail className="w-4 h-4 text-muted-foreground mr-1" />
        <span className="text-sm">Continue with email</span>
      </Button>
    </div>
  )
}
