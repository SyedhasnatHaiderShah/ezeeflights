...
const sectionLabel =
  "text-xs uppercase tracking-wider text-foreground font-semibold mb-3";
...
export function FilterSidebar({
  ...
  return (
    <aside className="w-full bg-card border-r border-border h-full overflow-y-auto p-3 min-h-screen">
          ...
          <AccordionContent>
            ...
            <div className="space-y-3">
              {[
                ...
                <label
                  key={item.id}
                  className="flex items-center justify-between text-sm text-foreground font-medium"
                >
                  <span className="flex items-center gap-2">
                    <Checkbox
                      checked={item.checked}
                      onCheckedChange={(checked) =>
                        toggleStop(item.id as any, Boolean(checked))
                      }
                    />
                    {item.label}
                  </span>
                  <span className="rounded-full bg-muted px-2 py-0.5 text-xs text-foreground font-bold">
                    {item.count}
                  </span>
                </label>
              ))}
          ...
          <AccordionContent>
            ...
            <div className="space-y-3">
              {visibleAirlines.map((airline) => (
                <label
                  key={airline.name}
                  className="flex items-center justify-between gap-2 text-sm text-foreground font-medium"
                >
                  <span className="flex items-center gap-2 min-w-0">
                    <Checkbox
                      checked={filters.airlines.includes(airline.name)}
                      onCheckedChange={(checked) =>
                        toggleAirline(airline.name, Boolean(checked))
                      }
                    />
                    <img
                      src={`https://www.kayak.com/rimg/provider-logos/airlines/v/${airline.code}.png`}
                      alt={airline.name}
            ...
            {airlines.length > 6 && (
              <button
                type="button"
                className="text-xs text-redmix mt-3 font-semibold hover:underline"
        ...
        {(["departure", "arrival"] as const).map((type) => {
          ...
          return (
            <AccordionItem key={type} value={type}>
              <AccordionTrigger>
                {type === "departure" ? "Departure time" : "Arrival time"}
              </AccordionTrigger>
              <AccordionContent>
                  ...
                  {timeBuckets.map((slot) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "rounded-lg border px-3 py-2 text-xs flex items-center gap-2 transition-all",
                          active
              ...
              {["Economy", "Premium Economy", "Business", "First"].map(
                (cabin) => (
                  ...
                  >
                    <input
                      type="radio"
                      checked={filters.cabinClass[0] === cabin}
                      onChange={() => setFilter("cabinClass", [cabin])}
                    />
                    {cabin}
                  </label>
                ),
        ...
        <AccordionItem value="duration">
          ...
          <AccordionContent>
            ...
            <p className="text-sm text-foreground font-bold mb-2">
              Up to {Math.round((filters.durationRange[1] / 100) * 24)} hours
            </p>
            <Slider
      ...
      <div className=" bg-card py-3 mt-4 border-t border-border flex items-center justify-between gap-2">
        <button
          type="button"
          onClick={() => {
            resetFilters();
