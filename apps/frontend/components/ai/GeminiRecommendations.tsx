...
export async function GeminiRecommendations({
  destinationCode,
  searchParamsString = "",
}: {
  ...
  const cityName =
    destMap[destinationCode?.toUpperCase()] ||
    destinationCode ||
    "your destination";
    ...
    return (
      <div className="w-full dark:bg-muted/10 border-dashed border-2 border-border p-5 rounded-3xl mb-8 flex flex-col items-center justify-center text-center">
        ...
        <h3 className="font-bold text-base text-foreground font-display">
          Aero Intelligence Offline
        </h3>
        <p className="font-medium text-sm text-muted-foreground w-full max-w-md mt-2">
          Unable to generate travel insights for {cityName}. Please configure
  ...
  try {
    ...
    console.warn(
      "Aero Intelligence Offline or API Key Invalid:",
      error.message,
    );
    return null;
  }
        ...
        <div className="flex justify-between items-end mb-6 pl-1">
          <div>
            ...
            <p className="text-muted-foreground mt-2 font-medium">
              Curated by our intelligence engine for elite comfort in {cityName}
              .
            </p>
          </div>
          <Link href={`/flights/stays?${searchParamsString}` as any}>
            <Button
              variant="ghost"
              className="text-[#ff4b2b] hover:text-[#ff4b2b] hover:bg-brand-red/5 font-bold flex items-center gap-2 hover:gap-3 transition-all h-10 px-4 group"
            >
              View Stays{" "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {aiData.hotels?.slice(0, 3).map((hotel: any, idx: number) => (
            <div key={`hotel-${idx}`} className="group cursor-pointer">
              ...
              <h3 className="font-bold text-lg mb-1 text-foreground font-display">
                {hotel.name}
              </h3>
              <p className="font-medium text-xs text-muted-foreground line-clamp-2">
                {hotel.description}
              </p>
            </div>
          ))}
        ...
        <div className="flex justify-between items-end mb-6 pl-1">
          <div>
            <h2 className="text-4xl font-bold tracking-tight text-foreground font-display flex items-center gap-3">
              ...
              <Utensils className="h-6 w-6 text-[#ff4b2b]" />
            </h2>
            <p className="text-muted-foreground mt-2 font-medium">
              Top-rated dining spots hand-picked for your layout in {cityName}.
            </p>
          </div>
          <Link href={`/flights/restaurants?${searchParamsString}` as any}>
            <Button
              variant="ghost"
              className="text-[#ff4b2b] hover:text-[#ff4b2b] hover:bg-brand-red/5 font-bold flex items-center gap-2 hover:gap-3 transition-all h-10 px-4 group"
            >
        ...
        <div className="flex justify-between items-end mb-6 pl-1">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm font-medium">
              Precision forecasting for tactical planning.
            </p>
          </div>
          <Link href={`/flights/places?${searchParamsString}` as any}>
            <Button
              variant="ghost"
              className="text-[#ff4b2b] hover:text-[#ff4b2b] hover:bg-brand-red/5 font-bold flex items-center gap-2 hover:gap-3 transition-all h-10 px-4 group whitespace-nowrap"
            >
              Explore Attractions{" "}
              <MapPin className="h-4 w-4 group-hover:translate-y-[-2px] transition-transform ml-1" />
            </Button>
          </Link>
        </div>
            ...
            return (
              ...
              >
                ...
                <WeatherIcon
                  ...
                  className={cn(
                    "h-7 w-7",
                    isToday ? "text-white" : "text-[#ff4b2b]",
                  )}
                />
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="bg-muted/20 h-48 rounded-3xl animate-pulse"
            />
          ))}
        </div>
      </div>
