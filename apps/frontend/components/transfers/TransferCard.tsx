    ...
    <article className="overflow-hidden rounded-2xl border border-border/80 bg-white shadow-sm">
      ...
      <div className="space-y-3 p-4">
        <h3 className="font-semibold capitalize">
          {transfer.vehicleType.replace("_", " ")}
        </h3>

        <div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground">
          <p className="inline-flex items-center gap-1">
            <Users className="h-4 w-4" /> {transfer.maxPassengers} pax
          </p>
          <p className="inline-flex items-center gap-1">
            <Luggage className="h-4 w-4" /> {transfer.maxLuggage} bags
          </p>
          <p className="inline-flex items-center gap-1">
            <MapPinned className="h-4 w-4" /> 24 km
          </p>
          <p className="inline-flex items-center gap-1">
            <Timer className="h-4 w-4" /> ETA 35 min
          </p>
        </div>
        ...
        <div className="flex items-end justify-between border-t border-border/70 pt-3">
          ...
          />
          <Link
            href={`/transfers/${transfer.id}?${queryString}` as any}
