...
export async function scanPassportDocument(file: File) {
  ...
  const response = await fetch('/api/travel-documents/passport/scan', {
    method: 'POST',
    credentials: 'include',
    headers: csrfHeaders('POST'),
    body: form,
...
export async function downloadTravelDocument(id: string) {
  const response = await fetch(`/api/travel-documents/${id}/download`, {
    credentials: 'include',
  });
...
export async function downloadSharedTravelDocument(token: string) {
  const response = await fetch(`/api/travel-documents/share/${token}`, {
    credentials: 'include',
  });
