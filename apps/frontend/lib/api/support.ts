...
export const getTickets=(status?:string,page=1)=>apiFetch<SupportTicket[]>(`/support/tickets/me?status=${status??''}&page=${page}`);
...
export const closeTicket=(ticketId:string)=>apiFetch<void>(`/support/tickets/${ticketId}/close`,{method:'POST'});
