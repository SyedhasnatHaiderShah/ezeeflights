...
}) => apiFetch<PaymentIntent>('/payments/initiate', { method: 'POST', body: JSON.stringify(payload) });
export const confirmPayment=(paymentIntentId:string)=>apiFetch<PaymentResult>(`/payments/${paymentIntentId}/confirm`,{method:'POST',body:JSON.stringify({})});
export const getWalletBalance=()=>apiFetch<WalletBalance>('/payments/wallet/me');
export const addWalletFunds=(amount:number,currency:string)=>apiFetch<WalletTransaction>('/payments/wallet/topup',{method:'POST',body:JSON.stringify({amount,currency})});
