...
describe('PaymentService', () => {
  ...
  const events = {
    emit: jest.fn(),
  } as unknown as jest.Mocked<AppEventBus>;
  ...
  const service = new PaymentService(repository, notification, events, [provider]);
  ...
  it('processes webhook and triggers booking/notification/event dependencies', async () => {
    ...
    expect(events.emit).toHaveBeenCalledWith('payment.succeeded', expect.objectContaining({ paymentId: 'p1', userId: 'u1' }));
