...
export class AuthController {
  ...
  @ApiOperation({ summary: 'Verify native Google ID token from Capacitor' })
  @ApiResponse({ status: 200, description: 'Login successful, returns tokens' })
  @ApiResponse({ status: 401, description: 'Invalid or expired ID token' })
  @Post('google/native')
  verifyNativeGoogleToken(@Body('idToken') idToken: string, @Res({ passthrough: true }) res: Response) {
    return this.authService.verifyNativeGoogleToken(idToken, res);
  }
  ...
  async googleCallback(@Req() req: Request & { user: OAuthUser }, @Res() res: Response): Promise<void> {
    const stateRedirectUri = req.query.state as string;
    try {
      const url = await this.authService.completeGoogleOAuthForBrowserRedirect({
        email: req.user?.email ?? '',
        provider: 'google',
        providerUserId: req.user?.providerId ?? '',
        customRedirectUri: stateRedirectUri,
      });
      res.redirect(302, url);
    } catch (err) {
      let base = 'http://localhost:3000';
      try {
        base = this.authService.getRedirectBase(stateRedirectUri);
      } catch {
        // Ignore fallback resolution error
      }
      res.redirect(302, base.includes('/auth/callback')
        ? `${base.split('?')[0]}?error=oauth_failed`
        : `${base}/auth/callback?error=oauth_failed`
      );
    }
  }
