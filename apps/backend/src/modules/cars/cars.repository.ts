...
export class CarRepository {
  ...
  async createBooking(booking: Omit<CarBooking, 'id' | 'createdAt' | 'updatedAt'>): Promise<CarBooking> {
    const created = await this.db.queryOne<CarBooking>(
      `INSERT INTO car_bookings (
          user_id, car_id, travelport_car_id, travelport_car_name, travelport_vendor_code,
       ...
       RETURNING id,
          user_id as "userId",
          car_id as "carId",
          travelport_car_id as "travelportCarId",
          travelport_car_name as "travelportCarName",
          travelport_vendor_code as "travelportVendorCode",
      ...
      [
        booking.userId,
        booking.carId ?? null,
        (booking as any).travelportCarId ?? null,
        (booking as any).travelportCarName ?? null,
        (booking as any).travelportVendorCode ?? null,
  ...
  async findLocationList(query?: string): Promise<CarLocation[]> {
    ...
    if (query?.trim()) {
      ...
      where = `WHERE name LIKE $1 OR city LIKE $1`;
    }
