  ...
  private async sendEmailBySendGrid(to: string, subject: string, html: string, text: string) {
    if (process.env.SMTP_HOST && process.env.SMTP_PORT) {
      try {
        ...
        const from = process.env.SMTP_FROM_EMAIL ?? process.env.SMTP_USER ?? process.env.EMAIL_FROM ?? 'noreply@ezeeflights.com';
        await transporter.sendMail({
          from: `Ezeeflights <${from}>`,
          to,
          subject,
          text,
          html,
        ...
        appLogger.error(`SMTP email failed: ${err.message}`);
      }
    }

    if (!process.env.SENDGRID_API_KEY) {
      appLogger.warn('SendGrid/SMTP not configured');
