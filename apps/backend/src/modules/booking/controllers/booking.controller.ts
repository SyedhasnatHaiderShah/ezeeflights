...
export class BookingController {
  ...
  @ApiResponse({
    status: 201,
    description: "Booking held and payment intent created",
  })
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  @Post("flights/hold")
  ...
  @ApiQuery({
    name: "type",
    required: false,
    description: "Booking type (flight, hotel, etc.)",
  })
  @ApiQuery({
    name: "status",
    required: false,
  ...
  @ApiOperation({
    summary: "Download trip document (ticket/voucher/insurance)",
  })
  @ApiParam({ name: "id", description: "Booking UUID" })
  @ApiQuery({
    name: "docType",
    required: false,
    enum: ["ticket", "voucher", "insurance"],
  ...
  async tripDocument(
    ...
    const document = await this.service.getTripDocument(
      req.user.userId,
      id,
      docType,
    );
  ...
  cancel(
    ...
    return this.bookingMgmtService.cancelBooking(
      id,
      req.user.userId,
      body,
