...
export class BookingRepository {
  ...
  private async getDefaultCurrencyFields() {
    try {
      ...
      return {
        ...
        bookingsSimple: tables.has('bookings') ? 'default_currency as "defaultCurrency"' : 'currency as "defaultCurrency"',
      };
    } catch {
      return {
        bookings: 'b.currency',
        ...
        bookingsSimple: 'currency as "defaultCurrency"',
      };
    }
  }
  ...
  async create(
    ...
    return this.db.withTransaction(async (client) => {
      // Load user and their preferred currency (if set)
      const userRow = await client.query(
        "SELECT id, preferred_currency FROM tbl_users WHERE id = $1 LIMIT 1",
        [userId],
      );
      ...
      const userPreferredCurrency =
        userRow.rows[0]?.preferred_currency ?? null;
      ...
      const flightCurrency = flightsRes.rows[0].currency as string;
      ...
      const resourceCurrency = validateCurrency(flightCurrency, flightCurrency);
      const bookingRes = await client.query(
        `INSERT INTO bookings (user_id, status, payment_status, total_amount, total_price, currency, default_currency)
         ...
         RETURNING id, user_id as "userId", status, payment_status as "paymentStatus",
           ...
           currency, default_currency as "defaultCurrency", created_at as "createdAt", updated_at as "updatedAt"`,
        [userId, status, paymentStatus, total, total, currency, resourceCurrency],
      );
    ...
    if (txClient) {
      const result = await txClient.query(
        `SELECT b.id, b.user_id as "userId", b.status, b.payment_status as "paymentStatus",
            ...
            b.currency, b.default_currency as "defaultCurrency", b.created_at as "createdAt", b.updated_at as "updatedAt"
         FROM bookings b
  ...
  async listByUser(userId: string): Promise<BookingEntity[]> {
    ...
    return this.db.query<BookingEntity>(
      `SELECT id, user_id as "userId", status, payment_status as "paymentStatus",
        total_amount::float8 as "totalAmount", total_price::float8 as "totalPrice", currency, ${fields.bookingsSimple},
        created_at as "createdAt", updated_at as "updatedAt"
       FROM bookings
       WHERE user_id = $1
    ...
    const rows = await this.db.query<TripSummaryEntity>(
      `SELECT * FROM (
        SELECT b.id,
          ...
          NULL::text as "hotelId",
          NULL::text as "hotelName",
          NULL::text as "hotelCity",
          NULL::text as "hotelCountry",
          COALESCE(bp.count, 0) as "guestCount",
          LOWER(b.payment_status) as "paymentStatus",
        ...
        SELECT hb.id,
          ...
          COALESCE(h.name, hb.hotel_id::text) as title,
          CONCAT(
            COALESCE(NULLIF(TRIM(BOTH ', ' FROM CONCAT_WS(', ', h.city, h.country)), ''), 'Hotel stay'),
            ' · ',
            COALESCE(hg.cnt, 0)::text, ' guest(s)',
            ' · Payment ', LOWER(hb.payment_status)
          ) as subtitle,
          hb.hotel_id::text as "hotelId",
          h.name as "hotelName",
          h.city as "hotelCity",
          h.country as "hotelCountry",
          COALESCE(hg.cnt, 0) as "guestCount",
          LOWER(hb.payment_status) as "paymentStatus",
          hb.created_at::text as "createdAt"
        FROM hotel_bookings hb
        LEFT JOIN hotels h ON h.id::text = hb.hotel_id::text
        LEFT JOIN LATERAL (
          SELECT COUNT(*)::int as cnt FROM booking_guests bg WHERE bg.booking_id = hb.id
        ) hg ON true
        WHERE hb.user_id = $1::uuid
        ...
        SELECT tb.id,
          ...
          NULL::text as "hotelCity",
          NULL::text as "hotelCountry",
          COALESCE(tb.passenger_count, 0) as "guestCount",
          NULL::text as "paymentStatus",
          tb.created_at::text as "createdAt"
        FROM transfer_bookings tb
        WHERE tb.user_id = $1::uuid
        ...
        WHERE pb.user_id = $1::uuid
        UNION ALL

        SELECT inq.id,
          inq.user_id as "userId",
          'inquiry'::text as type,
          LOWER(inq.status) as status,
          CONCAT('INQ-', UPPER(LEFT(REPLACE(inq.id::text, '-', ''), 8))) as "confirmationCode",
          COALESCE(inq.flight_snapshot->>'currency', 'USD')::text as currency,
          NULL::text as "defaultCurrency",
          COALESCE(NULLIF(inq.flight_snapshot->>'totalCost', '')::float8, NULLIF(inq.flight_snapshot->>'totalFare', '')::float8, 0)::float8 as total,
          COALESCE(inq.depart_date::text, inq.created_at::text) as "startDate",
          COALESCE(inq.depart_date::text, inq.created_at::text) as "endDate",
          CONCAT(inq.origin, ' → ', inq.destination) as title,
          CONCAT(COALESCE(inq.depart_date::text, 'No Date'), ' • ', COALESCE(inq.cabin_class, 'Economy')) as subtitle,
          NULL::text as "hotelId",
          NULL::text as "hotelName",
          NULL::text as "hotelCity",
          NULL::text as "hotelCountry",
          0 as "guestCount",
          NULL::text as "paymentStatus",
          inq.created_at::text as "createdAt"
        FROM flight_inquiries inq
        WHERE inq.user_id = $1::uuid
      ) all_trips
      WHERE "userId" = $1::uuid
  ...
  async getTripById(
    ...
    const [trip] = await this.db.query<TripSummaryEntity>(
      `SELECT * FROM (
        SELECT b.id, 'flight'::text as type, LOWER(b.status) as status,
          COALESCE(tp.pnr_code, CONCAT('FLT-', UPPER(LEFT(REPLACE(b.id::text, '-', ''), 8)))) as "confirmationCode",
          b.currency, ${fields.bookings} as "defaultCurrency", b.total_amount::float8 as total,
          COALESCE(f.departure_at::text, b.created_at::text) as "startDate",
          COALESCE(f.arrival_at::text, f.departure_at::text, b.created_at::text) as "endDate",
          COALESCE(CONCAT(f.departure_airport, ' → ', f.arrival_airport), 'Flight booking') as title,
          CONCAT(COALESCE(bp.count, 0), ' passenger(s)') as subtitle,
        ...
        SELECT hb.id, 'hotel'::text, LOWER(hb.status), CONCAT('HTL-', UPPER(LEFT(REPLACE(hb.id::text, '-', ''), 8))), hb.currency, ${fields.hotel_bookings} as "defaultCurrency",
          hb.total_price::float8, hb.check_in_date::text, hb.check_out_date::text,
          COALESCE(h.name, hb.hotel_id), COALESCE(NULLIF(CONCAT(h.city, ', ', h.country), ', '), 'Hotel reservation'), hb.created_at::text
        FROM hotel_bookings hb
        LEFT JOIN hotels h ON h.id::text = hb.hotel_id::text
        ...
        SELECT cb.id, 'car'::text, LOWER(cb.status::text), COALESCE(cb.confirmation_code, CONCAT('CAR-', UPPER(LEFT(REPLACE(cb.id::text, '-', ''), 8)))),
          cb.currency, ${fields.car_bookings} as "defaultCurrency", cb.total_price::float8, cb.pickup_datetime::text, cb.dropoff_datetime::text,
        ...
        SELECT tb.id, 'transfer'::text, LOWER(tb.status::text), COALESCE(tb.confirmation_code, CONCAT('TRF-', UPPER(LEFT(REPLACE(tb.id::text, '-', ''), 8)))),
          tb.currency, ${fields.transfer_bookings} as "defaultCurrency", tb.price::float8, tb.pickup_datetime::text, tb.pickup_datetime::text,
        ...
        SELECT pb.id, 'package'::text, LOWER(pb.booking_status), CONCAT('PKG-', UPPER(LEFT(REPLACE(pb.id::text, '-', ''), 8))),
          pb.currency, ${fields.package_bookings} as "defaultCurrency", pb.total_amount::float8, pb.created_at::text, pb.created_at::text,
    ...
    if (trip.type === "hotel") {
      const [hotel] = await this.db.query<TripDetailEntity["hotel"]>(
        `SELECT COALESCE(h.name, hb.hotel_id) as "propertyName",
            hb.check_in_date::text as "checkInDate",
            hb.check_out_date::text as "checkOutDate",
            COALESCE(r.room_type, br.room_id, 'Standard Room') as "roomType",
            COALESCE(h.address, NULLIF(CONCAT(h.city, ', ', h.country), ', '), hb.hotel_id) as address,
            CONCAT('https://maps.google.com/?q=', REPLACE(COALESCE(h.address, CONCAT(h.city, ' ', h.country), hb.hotel_id), ' ', '+')) as "mapUrl"
         FROM hotel_bookings hb
         LEFT JOIN hotels h ON h.id::text = hb.hotel_id::text
         LEFT JOIN booking_rooms br ON br.booking_id = hb.id
         LEFT JOIN rooms r ON r.id::text = br.room_id::text
         WHERE hb.id = $1
         LIMIT 1`,
        [bookingId],
      );
  ...
  async cancelTripById(
    ...
    const flight = await this.db.query<{ id: string }>(
      `UPDATE bookings
       ...
       WHERE id = $1 AND user_id = $2 AND LOWER(status) IN ('pending', 'confirmed', 'payment_pending', 'held')
       RETURNING id`,
      [bookingId, userId],
    );
    ...
    const hotel = await this.db.query<{ id: string }>(
      `UPDATE hotel_bookings
       SET status = 'CANCELLED', updated_at = NOW()
       WHERE id = $1 AND user_id = $2 AND LOWER(status) IN ('pending', 'confirmed', 'payment_pending')
       RETURNING id`,
      [bookingId, userId],
    );
    ...
    const car = await this.db.query<{ id: string }>(
      `UPDATE car_bookings
       ...
       WHERE id = $1 AND user_id = $2 AND LOWER(status::text) IN ('pending', 'confirmed', 'active')
       RETURNING id`,
    ...
    const transfer = await this.db.query<{ id: string }>(
      `UPDATE transfer_bookings
       ...
       WHERE id = $1 AND user_id = $2 AND LOWER(status::text) IN ('pending', 'confirmed', 'driver_assigned')
       RETURNING id`,
      [bookingId, userId],
    );
    ...
    const pkg = await this.db.query<{ id: string }>(
      `UPDATE package_bookings
       ...
       WHERE id = $1 AND user_id = $2 AND LOWER(booking_status) IN ('initiated', 'payment_pending', 'confirmed')
       RETURNING id`,
      [bookingId, userId],
    );
    if (pkg.length > 0) return;

    const inq = await this.db.query<{ id: string }>(
      `UPDATE flight_inquiries
       SET status = 'CLOSED', updated_at = NOW()
       WHERE id = $1 AND user_id = $2 AND LOWER(status) IN ('pending', 'reviewed', 'contacted')
       RETURNING id`,
      [bookingId, userId],
    );
