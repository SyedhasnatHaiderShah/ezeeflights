...
export interface FlightBookingPaymentRecord {
  id: string;
  userId: string | null;
  amount: number;
  currency: string;
...
@Injectable()
export class FlightBookingPaymentRepository {
  constructor(private readonly dataSource: DataSource) {}

  private mapRow(row: Record<string, unknown>): FlightBookingPaymentRecord {
    const metadata =
  ...
  }): Promise<FlightBookingPaymentRecord | null> {
    const id = randomUUID();
  ...
  async getByOrderId(orderId: string): Promise<FlightBookingPaymentRecord | null> {
