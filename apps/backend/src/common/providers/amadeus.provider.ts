...
export class AmadeusProvider {
  ...
  constructor(private readonly logger: PinoLogger) {
    if (!process.env.AMADEUS_CLIENT_ID) {
      console.warn('Missing required env var: AMADEUS_CLIENT_ID. Using a dummy fallback.');
    }
    if (!process.env.AMADEUS_CLIENT_SECRET) {
      console.warn('Missing required env var: AMADEUS_CLIENT_SECRET. Using a dummy fallback.');
    }
    this.client = new Amadeus({
      clientId: process.env.AMADEUS_CLIENT_ID || 'mock_client_id',
      clientSecret: process.env.AMADEUS_CLIENT_SECRET || 'mock_client_secret',
