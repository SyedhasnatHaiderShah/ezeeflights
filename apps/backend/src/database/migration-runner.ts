    ...
    try {
      ...
      for (const file of files) {
        if (!only) {
          ...
          if (exists) {
            appLogger.info(`Migration ${file} - SKIPPED (Already applied)`);
            continue;
          }
        }

        appLogger.info(`Migration ${file} - RUNNING`);
        try {
          ...
          appLogger.info(`Migration ${file} - PASSED`);
        } catch (error) {
          const err = error as Error;
          appLogger.error(`Migration ${file} - FAILED. Error: ${err.message}`);
          throw error;
        }
      }
    } catch (error) {
