...
async function ensurePublicReviewsSchema() {
  ...
  await pool.query('CREATE INDEX IF NOT EXISTS idx_public_reviews_created ON public_reviews(created_at DESC)');
  await pool.query('CREATE INDEX IF NOT EXISTS idx_public_reviews_rating_created ON public_reviews(rating DESC, created_at DESC)');
  await pool.query('CREATE INDEX IF NOT EXISTS idx_public_reviews_verified ON public_reviews(is_verified, created_at DESC)');
}
...
async function seed() {
  ...
  try {
    ...
    // 4. Seed Public Reviews
    await ensurePublicReviewsSchema();

    for (let i = 0; i < PUBLIC_REVIEWS_SEED.length; i++) {
      const review = PUBLIC_REVIEWS_SEED[i];

      const avatar = `https://i.pravatar.cc/180?img=${(i % 60) + 1}`;

      await pool.query(
        'DELETE FROM public_reviews WHERE author_name = $1 AND text = $2',
    ...
    console.log('✅ Public reviews seeded');
